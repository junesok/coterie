...
export async function GET() {
  try {
    ...
    const user = await prisma.user.findUnique({
      ...
      select: { id: true, name: true, email: true, username: true, theme: true },
    });
...
// PUT /api/users/me — 이름 / 비밀번호 / 테마 변경
export async function PUT(req: NextRequest) {
  try {
    ...
    const { name, currentPassword, newPassword, theme } = await req.json();
    const updateData: { name?: string; passwordHash?: string; theme?: string } = {};
